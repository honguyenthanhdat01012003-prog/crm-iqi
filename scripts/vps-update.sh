...
echo "→ Sau build: restart keeper (aaPanel Stop/Start hoặc bash scripts/watchdog-crm.sh)"
echo "→ Cron 1 phút: bash /www/wwwroot/crm-iqi/scripts/watchdog-crm.sh"
echo "→ Hard-refresh CRM (Ctrl+F5). Check: curl -s https://YOUR_DOMAIN/api/version"
echo "   version phải là 2026-07-16-sheet-sync-lite-h"
